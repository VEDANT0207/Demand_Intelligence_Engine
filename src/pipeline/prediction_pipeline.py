...
class PredictionPipeline:
    ...
    def load_artifacts(self):
        ...
        self.customer_training_columns = joblib.load(self.config["customer_training_columns_path"])
        self.sales_training_columns = joblib.load(self.config["sales_training_columns_path"])
    ...
    def prepare_customer_features(self, input_df):
        ...
        customer_features = customer_features.reindex(
            columns=self.customer_training_columns,
            fill_value=0
        )
    ...
    def prepare_sales_features(self,input_df,customer_predictions):
        ...
        sales_features[
            self.PREDICTED_CUSTOMERS_COLUMN
        ...
        sales_features = sales_features.reindex(
            columns=self.sales_training_columns,
            fill_value=0
        )
    ...
    def predict(self, input_df):
        ...
        customer_predictions = self.predict_customers(
            customer_features
        )

        sales_features = self.prepare_sales_features(
            input_df,
            customer_predictions
        )

        sales_predictions = self.predict_sales(
            sales_features
        )
